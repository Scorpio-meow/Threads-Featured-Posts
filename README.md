...
- 頁面頂部設有熱門標籤篩選列，依標籤頻率降序排列，預設僅顯示前 10 個最熱門標籤。每個標籤同時顯示其出現次數 (如 `#程式 (12)`)。
...
- **頁面資訊顯示**：即時顯示「第 X / Y 頁 · 共 Z 則貼文」資訊。無貼文時顯示「尚無精選貼文」。
...
- **原因**：以下任一情況均可能導致 embed 無法正常渲染：
  - 貼文已被作者刪除或改為私密（需登入且為追蹤者才可查看）
  - 瀏覽器目前未登入 Threads，而 Threads 對該貼文強制要求登入狀態才能存取
  - 瀏覽器安全性限制（`X-Frame-Options: deny`）阻止了 iframe 載入
  - Threads 官方 embed.js 回傳異常高度（低於 200px）或載入逾時（超過 20 秒）
- **解決方案**：
  - 若為登入問題，可在同一瀏覽器開新分頁前往 [threads.com](https://www.threads.com) 完成登入後重新整理頁面，使 embed.js 能帶上 Cookie 向 Threads 伺服器請求內容。
  - 若貼文已刪除或設為私密，則無法透過嵌入方式取得內容，建議從 `config.js` 移除該筆資料以維持展示品質。
  - 如需排查具體原因，可在 URL 加上 `?debug=1` 參數並開啟瀏覽器開發者工具（`F12`），觀察 Console 中的詳細錯誤訊息。
  - 此降級為預期行為，呈現的備用連結允許使用者直接前往 Threads 官方網站瀏覽原始貼文。

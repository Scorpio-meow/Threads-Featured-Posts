...
- **小批次併發限制**：在載入貼文時，系統不會一次性載入整頁。而是依 [config.js](./config.js) 中定義的 `BATCH_SIZE`（預設為 3，強制限制在 1 至 4 之間）作為併發上限。每篇貼文的初始化啟動時間以 `EMBED_STAGGER_DELAY`（預設 700 毫秒）錯開，並加入隨機抖動（Jitter）避免請求同步化。
